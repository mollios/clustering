...
int main(void){
  ...
  Qmfa test(data_dimension, data_number, centers_number,
  ...
  while(1){
    ...
    if(std::isnan(diff_u) || std::isnan(diff_u)){
      std::cout << Em<<"\t"<<Lambda<< "\t" << std::endl;
        return 0;
    }
  ...
  Qmfa ClassFunction(test.dimension(), 1, test.centers_number(),
