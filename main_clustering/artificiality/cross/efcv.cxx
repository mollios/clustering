...
int main(void){
  ...
  Efcv test(data_dimension, data_number, centers_number,
            sub_dimension, Lambda);
#ifdef VERBOSE
  std::cout << test.basis(0).cols() << "\t"
            << test.basis(0).rows() << std::endl;
#endif  
  ...
  Efcv ClassFunction(test.dimension(), 1, test.centers_number(),
