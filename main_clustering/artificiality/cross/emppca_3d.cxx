...
int main(void){
  ...
  std::string filenameData("./data/test/debug_3d.dat");
#ifdef CHECK_ANSWER
  std::string filenameCorrectCrispMembership
    ("./data/test/debug_3d.correctCrispMembership");
#endif
  ...
  Emppca test(data_dimension, data_number, centers_number,
  ...
  test.basis(0,0,0)=0.0;
  test.basis(0,1,0)=1.0;
  test.basis(0,2,0)=0.0;
  test.basis(0,0,1)=1.0;
  test.basis(0,1,1)=0.0;
  test.basis(0,2,1)=0.0;
  test.basis(1,0,0)=0.0;
  test.basis(1,1,0)=1.0;
  test.basis(1,2,0)=0.0;
  test.basis(1,0,1)=1.0;
  test.basis(1,1,1)=0.0;
  test.basis(1,2,1)=0.0;
  for(int i=0;i<test.centers_number();i++){
    test.sigma(i)=1.0;
  }
#ifdef VERBOSE
  ...
  while(1){
    ...
    double diff=diff_u+diff_v+diff_W+diff_sigma+diff_a;
#ifdef DIFF
    std::cout << "#diff:" << diff << "\t";
  ...
  Emppca ClassFunction(test.dimension(), 1, test.centers_number(),
  ...
  for(int i=0;i<test.centers_number();i++){
    ClassFunction.sigma(i)=test.sigma(i);
  }
  for(int i=0;i<test.centers_number();i++){
