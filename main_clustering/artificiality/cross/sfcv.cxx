...
int main(void){
  ...
  Sfcv test(data_dimension, data_number, centers_number,
            sub_dimension, Em);
#ifdef VERBOSE
  ...
  std::cout << Em<<"\t" << test.ARI() << std::endl;
#endif
  
#ifdef CRFILE
  std::string filenameResultMembership
    =std::string("./debug_data/sFCV/")
    +std::string("sFCV-")
  ...
  if(!ofs_basis){
    std::cerr << "File:" << filenameResultBasiss
	      << "could not open." << std::endl;
    exit(1);
  }
  for(int i=0;i<test.centers_number();i++){
    for(int l=0;l<test.basis(i).rows();l++){
      ...
      ofs_basis << std::endl;
      for(int ell=0;ell<test.dimension();ell++){
        ofs_basis << test.centers()[i][ell]+test.basis(i)[l][ell] << "\t";
      }
      ofs_basis << "\n\n"<< std::endl;
    }
  }
  ...
  Sfcv ClassFunction(test.dimension(), 1, test.centers_number(),
                     1, test.fuzzifierEm());
