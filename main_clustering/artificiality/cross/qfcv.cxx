...
int main(void){
  ...
  double strideEm=0.25;
  double stridelambda=2.0;
  std::string filenameData("./data/test/debug4.dat");
#ifdef CHECK_ANSWER
  std::string filenameCorrectCrispMembership
    ("./data/test/debug4.correctCrispMembership");
#endif
  ...
  for(int p=0;p<parameter_number;p++){
    Em=para_Em[p];
    for(int p2=0;p2<parameter_number;p2++){
      Lambda=para_Lambda[p2];
#endif
  ...
  Qfcv test(data_dimension, data_number, centers_number,
  ...
  while(1){
    ...
    double diff=diff_u+diff_v+diff_W+diff_a;
#ifdef DIFF
    std::cout << "#diff:" << diff << "\t";
  ...
  for(int i=0;i<test.centers_number();i++){
    for(int l=0;l<test.basis(i).rows();l++){
      for(int ell=0;ell<test.dimension();ell++){
        ofs_basis << test.centers()[i][ell] << "\t";
      }
      ofs_basis << std::endl;
      for(int ell=0;ell<test.dimension();ell++){
        ofs_basis << test.basis(i)[l][ell] << "\t";
      }
      ofs_basis << "\n\n" <<std::endl;
    }
    ofs_basis << std::endl;
  ...
  Qfcv ClassFunction(test.dimension(), 1, test.centers_number(),
                     1, test.fuzzifierEm(), test.fuzzifierLambda());
  ...
  for(int i=0;i<test.centers_number();i++){
    for(int l=0;l<test.basis(i).rows();l++){
      for(int ell=0;ell<test.dimension();ell++){
        ClassFunction.basis(i)[l][ell]=test.basis(i)[l][ell];
      }
    }
  }
  for(int i=0;i<test.centers_number();i++){
  ...
  int flag=0;
  int flag2=0;
  for(double x0=Min[0];x0<=Max[0];x0+=Width[0]/20.0){
    ...
    if(x0>=0.8&&flag!=0)
      x0-=0.85;
    for(double x1=Min[1];x1<=Max[1];x1+=Width[1]/20.0){
