...
int main(void){
  ...
  Efcv test(data_dimension, data_number, centers_number, sub_dimension, Lambda);
#ifdef VERBOSE
...
#endif
#ifdef WHILE
  ofs_ARI << Lambda << "\t" << test.ARI() << std::endl;
#endif
  
#ifdef CRFILE
  std::string filenameResultMembership
    =std::string("./mnist/eFCV/")
    +std::string("eFCV-")
    +std::to_string(test.fuzzifierLambda())+std::string("-")
    +filenameData.substr(filenameDataSlashPosition, filenameDataDotPosition)
    +std::string(".result_membership");
  std::ofstream ofs_membership(filenameResultMembership,ios::app);
  if(!ofs_membership){
